...
{
    public class AddUserViewModel: ViewModelBaseValidate
    {
        #region Конструктор
        ...
        public void AddUser() => _ = AddUserAsync();
        public async Task AddUserAsync()
        {
            if (!HasErrors)
            {
                UserModel userModel = DepartmentModel == null ?
                    await _userService.Create(new UserModel { Name = Name, Surname = Surname, Patronymic = Patronymic, Gender = Gender, DateOfBirth = DateOfBirthDateOnly }) :
                    await _userService.Create(new UserModel { Name = Name, Surname = Surname, Patronymic = Patronymic, Gender = Gender, DateOfBirth = DateOfBirthDateOnly, DepartmentId = DepartmentModel.Id });
